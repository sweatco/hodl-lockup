...
pub async fn add_oracle(ft: &Contract, account_id: &AccountId) -> Result<()> {
    ft.call("add_oracle")
...
pub async fn ft_transfer_call(ft: &Contract, from: &Account, to: &AccountId, amount: u128, msg: &str) -> Result<()> {
    from.call(ft.id(), "ft_transfer_call")
        .args_json(json!({
            "receiver_id": to,
